...
function AppContent() {
  ...
  const [currentPage, setCurrentPage] = useState<'home' | 'soil-analysis' | 'weather' | 'recommendations' | 'market-insights' | 'profile'>('home');
  ...
  const recentActivities = [
    { id: 1, type: 'soil_test', message: 'Soil analysis completed for Field A', time: '2 hours ago', icon: TestTube, color: 'text-amber-600' },
    { id: 2, type: 'weather_alert', message: 'Rain forecast for tomorrow', time: '4 hours ago', icon: Cloud, color: 'text-blue-600' },
    { id: 3, type: 'crop_recommendation', message: 'New crop suggestions available', time: '1 day ago', icon: Sprout, color: 'text-green-600' },
    { id: 4, type: 'market_update', message: 'Wheat prices increased by 5%', time: '2 days ago', icon: TrendingUp, color: 'text-purple-600' }
  ];
  ...
  const quickStats = [
    { label: 'Active Alerts', value: '3', icon: Bell, color: 'text-red-600', bgColor: 'bg-red-50' },
    { label: 'Tasks Today', value: '5', icon: CheckCircle, color: 'text-green-600', bgColor: 'bg-green-50' },
    { label: 'Crops Monitored', value: farmerProfile?.farmDetails?.currentCrops?.length || 0, icon: Sprout, color: 'text-blue-600', bgColor: 'bg-blue-50' },
    { label: 'Days Active', value: Math.floor((new Date().getTime() - new Date(farmerProfile?.createdAt || new Date()).getTime()) / (1000 * 60 * 60 * 24)), icon: Activity, color: 'text-purple-600', bgColor: 'bg-purple-50' }
  ];
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="text-center mb-8">
          ...
          {farmerProfile.farmLocation.address && (
            <div className="flex items-center justify-center mt-2 text-sm text-gray-500">
              <span>📍 {farmerProfile.farmLocation.district}, {farmerProfile.farmLocation.state}</span>
            </div>
          )}
        </div>
        ...
        {farmerProfile.farmDetails.totalArea > 0 && (
          <div className="bg-white rounded-xl shadow-lg p-6 mb-8">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">Your Farm Overview</h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="text-center p-4 bg-green-50 rounded-lg">
                <div className="text-2xl font-bold text-green-600">{farmerProfile.farmDetails.totalArea}</div>
                <div className="text-sm text-gray-600">Total Acres</div>
              </div>
              <div className="text-center p-4 bg-blue-50 rounded-lg">
                <div className="text-2xl font-bold text-blue-600">{farmerProfile.farmDetails.currentCrops.length}</div>
                <div className="text-sm text-gray-600">Crop Varieties</div>
              </div>
              <div className="text-center p-4 bg-purple-50 rounded-lg">
                <div className="text-2xl font-bold text-purple-600">{farmerProfile.farmDetails.soilType || 'N/A'}</div>
            ...
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <div className="flex items-center space-x-4 text-sm text-gray-500">
                  <span className="flex items-center"><Droplets className="h-4 w-4 mr-1" />{weatherData.humidity}%</span>
                  <span className="flex items-center"><Wind className="h-4 w-4 mr-1" />{weatherData.windSpeed} km/h</span>
                </div>
              </div>
            </div>
              ...
              {weatherData.forecast.map((day, index) => (
                <div key={index} className="flex items-center justify-between py-2 border-b border-gray-100 last:border-b-0">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-sm font-medium text-gray-900">{day.high}°/{day.low}°</span>
                  </div>
                </div>
              ))}
            </div>
